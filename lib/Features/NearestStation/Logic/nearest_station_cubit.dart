...
//   // ── select station (من marker أو card) ─────────────────────────────────────
...
//   /// custom marker — circle مع emoji بناءً على الـ type
...
//   // ── internal helper: fetch + rank + build markers ───────────────────────────
...
//     await _fetchAndEmit(
//       lat:          lat,          // ✅ coordinates الـ searched place
//       lng:          lng,
//       locationName: overrideName,
//       type:         state.selectedFilter,
//       // saveAsUserLocation: false (default) → userLat/userLng بتفضل بتاعة الـ user الحقيقي
//     );
//   }
...
//       return p != null ? stations[i].copyWith(crowding: p.crowdingLevel) : stations[i];
//     });
...
//     if (scored.isNotEmpty) scored[0] = scored[0].copyWith(isBestPrediction: true);
//     return scored;
//   }
...
class NearestStationCubit extends Cubit<NearestStationState> {
  ...
  Future<List<NearestStationModel>> _withPredictions(
    ...
    final withPred = List.generate(stations.length, (i) {
      final p = predictions[i];
      return p != null
          ? stations[i].copyWith(crowding: p.crowdingLevel)
          : stations[i];
    });
